...
const buildDocFormData = (file: SelectedMediaFile, sourceCode: string, targetCode: string) => {
    const formData = new FormData();
    formData.append('sourceCode', sourceCode);
    formData.append('targetCode', targetCode);

    if (Platform.OS === 'web' && file.webFile) {
        formData.append('document', file.webFile, file.name);
    } else {
...
export const requestTranslatedDoc = async ({
    file,
    sourceCode,
    targetCode,
}: {
    ...
    const response = await fetch(`${baseUrl}/api/docs/translate`, {
        method: 'POST',
        body: buildDocFormData(file, sourceCode, targetCode),
    });
    ...
    const fileName = nameMatch?.[1] || `translated-${targetCode}.txt`;
...
export const downloadOrShareTranslatedDoc = async ({
    fileName,
    mimeType,
    fileUri,
    webBlob,
    ...
    if (Platform.OS === 'web') {
        if (!webBlob) {
            throw new Error('Missing translated document blob for web download.');
        }
    ...
    await Sharing.shareAsync(fileUri, {
        mimeType,
        dialogTitle: 'Download translated document',
    });
};
